package main.java.org.yangbutao.longconn;
...
public class Connection extends Thread {
	...
	protected synchronized void setupIOstreams() throws IOException,
		...
		try {
			System.out.println("Connecting to " + remoteId);
			// 建立Connection的socket
			setupConnection();
			// 输入和输出流的初始化
	...
	protected void sendParam(Call call) {
		...
		try {
			System.out.println(getName() + " sending #" + call.id);
...
//			putInt(data, 0, dataLength - 4);
	...
	public void run() {
		...
		try {
			...
			System.out.println("Unexpected exception receiving call responses"+t.getMessage());
	...
	protected synchronized void close() {
		...
		if (!shouldCloseConnection.get()) {// shouldCloseConnection=false
			System.out.println("The connection is not in the closed state");
			return;
		}
		...
		if (closeException == null) {
			...
			// log the info
			System.out.println("closing ipc connection to " + remoteId.address
	...
	protected void receiveResponse() {
		...
		try {
			...
			do {
				datalength = in.read(recv);
			} while (datalength == 0);
			byte[] realData = new byte[datalength];
			System.arraycopy(recv, 0, realData, 0, datalength);
			System.out.println(datalength+":"+Arrays.toString(realData));
			BuildRequest.parse(realData);
			Call call = calls.remove(7013);
			if (call != null) {//
...
//			}
	...
	protected void cleanupCalls(long rpcTimeout) {
		...
		try {
			...
			System.out.println("Couldn't lower timeout, which may result in longer than expected calls");
		}
	}
	...
	private void handleConnectionFailure(int curRetries, int maxRetries,
		...
		System.out.println("Retrying connect to server: " + remoteId.getAddress()
